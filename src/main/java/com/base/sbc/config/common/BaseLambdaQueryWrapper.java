...
public class BaseLambdaQueryWrapper<T> extends LambdaQueryWrapper<T> {
    ...
    public <R> BaseLambdaQueryWrapper<T> notEmptyNotIn(SFunction<T, R> column, Collection<R> coll) {
    ...
    public BaseLambdaQueryWrapper<T> in(SFunction<T, ?> column, String str) {
        this.in(column, StrUtil.split(str, CharUtil.COMMA).stream().distinct().collect(Collectors.toList()));
        return this;
    }
